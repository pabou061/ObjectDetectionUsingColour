...
def on_trackbar(number):
    ...
    closing = cv2.morphologyEx(rgb, cv2.MORPH_CLOSE, kernel)
    erosion = cv2.erode(closing,kernel1,iterations = 1)
    res = cv2.dilate(erosion,kernel1,iterations = 1)
...
cv2.createTrackbar('trackbar', 'Processed Hue', 0 , 2, on_trackbar)
if cv2.waitKey(0) != -1:
    waitDone = 1
